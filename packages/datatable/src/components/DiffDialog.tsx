...
interface DiffDialogProps {
  ...
  onCommit: () => void;
  onRevertAll: () => void;
  onRevertField: (rowId: string, fieldKey: string, originalValue: unknown) => void;
  diffData: Array<{
    rowId: string;
    original: GridRow | undefined;
    edited: GridRow;
  }>;
}

/**
 * Dialog component to review and manage edited row changes
 * Memoized to prevent unnecessary re-renders
 */
export const DiffDialog = memo<DiffDialogProps>(
  ({ isOpen, onClose, onCommit, onRevertAll, onRevertField, diffData }) => {
      ...
      <div className="datatable-dialog-overlay">
        <div className="datatable-dialog">
          ...
          <div className="datatable-dialog-footer">
            <button onClick={onRevertAll} className="datatable-button-revert-all">
              Revert
            </button>
            <button onClick={onCommit} className="datatable-button-commit">
              Commit Changes
            </button>
          </div>
        </div>
