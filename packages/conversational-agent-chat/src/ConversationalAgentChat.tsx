...
export const ConversationalAgentChat = ({
  ...
  const getConversation =
    ...
    }, [chatService, existingConversationId, jobStartOverrides, resolveAgent, t]);
  ...
  const getSessionHelper = useCallback(async (): Promise<SessionStream> => {
    ...
    sessionHelper.onLabelUpdated(({ label }) => {
      if (!chatService) return;
      const idx = pastConversations.current.findIndex(
        (c) => c.id === conversation.id,
      );
      if (idx === -1) return;
      pastConversations.current = [
        ...pastConversations.current.slice(0, idx),
        { ...pastConversations.current[idx], label },
        ...pastConversations.current.slice(idx + 1),
      ];
      chatService.setHistory(
        getConversationHistoryDisplayItems(
          pastConversations.current,
