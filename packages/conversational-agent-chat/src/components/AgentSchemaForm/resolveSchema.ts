...
/**
 * Resolves a JSON Schema with $ref/$defs and anyOf/oneOf nullable patterns
 ...
 * 2. `anyOf|oneOf: [{type: "string"}, {type: "null"}]` — extracts the non-null type
 */
...
/**
 * Resolves a $ref and strips the auto-generated $defs title (e.g.
 * "DynamicType_1") so the consuming form falls back to the field key or
...
/**
 * Given an `anyOf` or `oneOf` array, extracts the non-null type if this is a
 * simple nullable pattern like `anyOf: [{type: "string"}, {type: "null"}]`.
 *
 * Also handles `$ref` entries like:
 * `anyOf: [{$ref: "#/$defs/Foo"}, {type: "null"}]`
 *
 * Also handles `const` entries (enum-like):
